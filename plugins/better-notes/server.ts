...
const SYNC_STATE_FILE = ".bb-better-notes-state.json";
const SYNC_STATE_VERSION = 1;

class CliUsageError extends Error {}

const DOCS_CLI_USAGE =
  "Usage: bb better-notes <vaults|vault-add|vault-remove|list|read|pull|status|push|write|mkdir|move|remove>";
const DOCS_STATUS_USAGE =
  "bb better-notes status [workspace-dir] [--delete] [--diff] [--workspace-host <id>] [--json]";
const DOCS_STATUS_HELP = [
  ...
  "",
  "Exit 4 is a successful status result. Review the output, then run bb better-notes push separately.",
].join("\n");
...
function parseCli(argv: string[]): {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (arg.startsWith("--") && allowedOptions && !allowedOptions.has(arg)) {
      throw new CliUsageError(`${arg} is not valid for bb better-notes ${command}`);
    }
...
function validateCliPositionals(args: ReturnType<typeof parseCli>): void {
  ...
  if (
    ...
    throw new CliUsageError(
      `bb better-notes ${args.command} received ${args.positionals.length} positional argument(s); expected ${
...
export default async function plugin(
  ...
  async function resolveOpenerFile(
    ...
    throw new Error(
      "Better Notes can open workspace, host, and thread-storage files only",
    );
  }
  ...
  async function runPull(
    ...
    const rootPath = resolveHostPath(
      cwd,
      args.into ?? `better-notes-${snapshot.vault.id}`,
    );
  ...
  async function runPushPlan(
    ...
    const rootPath = resolveHostPath(
      cwd,
      args.positionals[0] ?? args.into ?? `better-notes-${defaultVault.id}`,
    );
    if (args.positionals.length > 1) {
    ...
    if (!existing) {
      throw new Error(
        `${SYNC_STATE_FILE} was not found; run bb better-notes pull first`,
      );
    }
  ...
  function formatSyncHumanOutput(command: string, result: unknown): string {
    ...
    if (command === "status" || command === "push") {
      ...
      const lines = [
        `Better Notes workspace: ${String(result.rootPath)}`,
  ...
  bb.cli.register({
    name: "better-notes",
    summary: "Discover and safely sync Better Notes vaults",
    commands: [
      {
        name: "vaults",
        summary: "List configured vaults",
        usage: "bb better-notes vaults [--json]",
      },
      {
        name: "vault-add",
        summary: "Add a vault",
        usage: "bb better-notes vault-add <name> <absolute-root> [host-id]",
      },
      {
        name: "vault-remove",
        summary: "Remove a vault configuration",
        usage: "bb better-notes vault-remove <id>",
      },
      {
        name: "list",
        summary: "List notes and folders",
        usage: "bb better-notes list [--vault <id>] [--json]",
      },
      {
        name: "read",
        summary: "Read a file",
        usage: "bb better-notes read <path> [--vault <id>]",
      },
      {
        name: "pull",
        summary: "Pull one file, a folder subtree, or a whole vault",
        usage:
          "bb better-notes pull <path> [--folder] | --all [--vault <id>] [--into <dir>] [--workspace-host <id>] [--json]",
      },
      {
        name: "status",
      ...
      },
      {
        ...
        usage:
          "bb better-notes push [workspace-dir] [--delete] [--dry-run] [--diff] [--workspace-host <id>] [--json]",
      },
      {
        name: "write",
        summary: "Deprecated: write a UTF-8 file directly",
        usage: "bb better-notes write <path> --content <text> [--vault <id>]",
      },
      {
        name: "mkdir",
        summary: "Deprecated: create a folder directly",
        usage: "bb better-notes mkdir <path> [--vault <id>]",
      },
      {
        name: "move",
        summary: "Deprecated: move a path directly",
        usage: "bb better-notes move <from> <to> [--vault <id>]",
      },
      {
        name: "remove",
        summary: "Deprecated: remove a file or directory directly",
        usage: "bb better-notes remove <path> [--vault <id>] [--recursive]",
      },
    ],
    async run(argv, context) {
      ...
      try {
        ...
        if (args.command === "vaults") result = listVaults();
          ...
          warning =
            "Deprecated: direct Better Notes mutations will be removed; use bb better-notes pull, edit local files, then bb better-notes push.";
          ...
          warning =
            "Deprecated: direct Better Notes mutations will be removed; use bb better-notes pull, edit local files, then bb better-notes push --delete.";
        } else {
          return {
            exitCode: 2,
  ...
  bb.ui.registerMentionProvider({
    ...
    async resolve(itemId) {
      ...
      return {
        context: `Better Notes document (${vaultId}/${relativePath}):\n\n${file.content}`,
      };
    },
  });
