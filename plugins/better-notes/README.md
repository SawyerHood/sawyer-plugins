...
Better Notes is a filesystem-first document library for bb. Documents remain ordinary
...
A starting-point fork of BB’s built-in Docs / simple-notes plugin, copied from
`plugins/docs` at BB commit `6084f894305e389604ce19b1cace442b0fc25bad`. The upstream MIT
license is included in `LICENSE`. Shared UI components are vendored under `ui/`.

The plugin ID, navigation, CLI, mentions, and directive use `better-notes`,
so this can coexist with the built-in plugin. Settings are separate; the default
vault still points to `~/Notes`, so both plugins can access the same files.
...
- **Vaults on connected hosts:** each vault is a named `{ hostId, rootPath }`
  ...
  primary host. Manage local and remote vaults from the Better Notes panel.
...
- **Safe host-routed operations:** all list/read/write/mkdir/move/remove calls
  ...
  to polling.
- **Default Markdown editor:** Better Notes registers for `.md`, `.mdx`, and
...
- **YAML frontmatter:** an opening fenced block that parses as a YAML mapping
  ...
  instead keeps that content in the editor. Better Notes leaves the filename unchanged
...
- **Thread links:** agents can emit a Better Notes directive that renders as a document
  card. Clicking the card opens an editable, autosaving document in the thread
  side panel; its secondary action opens the full Better Notes editor. The side-panel
  editor can quote its selection (or full document) into the thread composer or
  insert a live Better Notes mention. These composer actions are intentionally absent
...
The plugin ships `skills/better-notes/SKILL.md`. Installed agents are taught to use the
Better Notes CLI, understand that a Better Notes `@`-mention is user-provided document context,
store plans and HTML artifacts in a vault when asked, and return `::better-notes` links
that the user can open in bb.

## CLI

The plugin registers the agent-discoverable `bb better-notes` command:

```sh
bb better-notes vaults --json
bb better-notes vault-add Work /home/me/work-better-notes host_workstation
bb better-notes list --vault personal --json
bb better-notes read projects/plan.md --vault personal
bb better-notes pull projects/plan.md --vault personal --into ./better-notes-work
# Edit ./better-notes-work/projects/plan.md with an ordinary editor or agent file tool.
bb better-notes status ./better-notes-work --diff
bb better-notes push ./better-notes-work

bb better-notes pull projects --folder --vault personal --into ./better-notes-work
bb better-notes pull --all --vault personal --into ./better-notes-work
bb better-notes push ./better-notes-work --dry-run --diff
bb better-notes push ./better-notes-work --delete
```

### Sync workspace contract

- **Layout and identity:** the destination keeps exact vault-relative paths
  beneath one workspace root and stores a versioned `.bb-better-notes-state.json`
...
- **Collisions:** exact relative paths are the stable identity. Pull rejects
  ...
  part of the accessible Better Notes contract and are skipped.
...
The stable internal plugin ID remains `better-notes`. Generate or inspect its
token with `bb plugin token better-notes`, then send it in
`x-bb-plugin-token` to these JSON endpoints:

```text
POST /api/v1/plugins/better-notes/http/list
POST /api/v1/plugins/better-notes/http/read
POST /api/v1/plugins/better-notes/http/write
POST /api/v1/plugins/better-notes/http/mkdir
POST /api/v1/plugins/better-notes/http/move
POST /api/v1/plugins/better-notes/http/remove
POST /api/v1/plugins/better-notes/http/sync/snapshot
POST /api/v1/plugins/better-notes/http/sync/apply
```
...
```sh
bb plugin install .
bb plugin config better-notes set directory "~/Notes"
bb plugin reload better-notes
```
