...
- A Better Notes panel with a folder tree, search, and a rich Markdown editor. Tables, images, and YAML frontmatter are supported.
...
Agents get the `better-notes` skill and the `bb better-notes` command. They list vaults with `bb better-notes vaults` and read files with `bb better-notes read`. They edit with `bb better-notes pull`, `bb better-notes status`, and `bb better-notes push`. Push uses version checks, so a concurrent edit is reported as a conflict instead of being overwritten.
