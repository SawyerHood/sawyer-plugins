...
describe("Better Notes nav panel", () => {
  it("remembers the document and restores it when the panel reopens", async () => {
    const slot = renderSlot(
      docsRegistration,
      { subPath: "personal/plans/My report.html" },
      { rpc: { listNotes: () => listNotesResult([]), preparePreview: () => preview } },
    );
    await slot.findByTitle("plans/My report.html");
    expect(localStorage.getItem("bb:better-notes:last-document")).toBe(
      "personal/plans/My report.html",
    );
    cleanup();
    ...
    await waitFor(() => expect(reopened.navigateCalls).toContainEqual({
      method: "toPluginPanel",
      path: "better-notes",
      options: { subPath: "personal/plans/My report.html", replace: true },
    }));
  });

  it("honors explicit vault navigation instead of restoring an old document", async () => {
    localStorage.setItem("bb:better-notes:last-document", "personal/old.md");
    const slot = renderSlot(
      docsRegistration,
      { subPath: "personal" },
      { rpc: { listNotes: () => listNotesResult([]) } },
    ...
    expect(localStorage.getItem("bb:better-notes:last-document")).toBeNull();
  });

  it("ignores malformed saved locations", async () => {
    localStorage.setItem("bb:better-notes:last-document", "%invalid");
    const slot = renderSlot(
      docsRegistration,
      { subPath: "" },
      { rpc: { listNotes: () => listNotesResult([]) } },
  ...
  it("registers the Better Notes surfaces", () => {
    ...
    expect(app.messageDirectives[0]?.id).toBe("better-notes");
  ...
  it("keeps task checkboxes aligned with the first line of their text", async () => {
    const existingStyles = document.head.querySelector(
      "style[data-bb-better-notes-styles]",
    );
  ...
  it("renders and autosaves editable Markdown tables", async () => {
    ...
    expect(table?.querySelector("td")?.textContent).toBe("Better Notes");
  ...
  it("opens Better Notes directive cards in the thread panel or full editor", () => {
    ...
    const slot = renderSlot(
      app.messageDirectives[0]!,
      {
        ...
        source:
          '::better-notes{vault="personal" path="plans/release.md" title="Release plan"}',
        message: {
          id: "msg_1",
          threadId: "thr_1",
          turnId: "turn_1",
    ...
    fireEvent.click(slot.getByRole("button", { name: "Open in Better Notes" }));
    expect(slot.navigateCalls).toContainEqual({
      method: "toPluginPanel",
      path: "better-notes",
  ...
  it("renders a linked Markdown document in the Better Notes thread panel", async () => {
    const slot = renderSlot(
