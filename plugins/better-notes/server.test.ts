...
async function loadNotebook(
  ...
  const directory = await mkdtemp(path.join(os.tmpdir(), "bb-better-notes-"));
...
describe("Better Notes RPC contract", () => {
  ...
  it("rejects invalid method inputs and outputs at runtime", async () => {
    const { bb, harness } = createFakePluginHost({ pluginId: "better-notes-contract" });
...
describe("Better Notes mention provider", () => {
  ...
  it("resolves the note's current content at send time", async () => {
    ...
    await expect(provider.resolve("personal:ideas.md")).resolves.toEqual({
      context:
        "Better Notes document (personal/ideas.md):\n\n# Fresh Ideas\n\nBuild the mention flow.",
    });
...
describe("Better Notes vault operations", () => {
  it.each([false, true])(
  ...
  it("registers the agent-discoverable Better Notes CLI", async () => {
    const { harness } = await loadNotebook({ "plan.md": "# Plan" });
  ...
  it("round-trips a folder edit through pull and push without changing binary assets", async () => {
    ...
    expect(files.has("/work/sync/.bb-better-notes-state.json")).toBe(true);
  ...
  it("fails closed on malformed sync state with machine-readable output", async () => {
    ...
    setUtf8("/work/sync/.bb-better-notes-state.json", "{not-json");
  ...
  it("renders top-level help successfully and rejects inapplicable flags before mutation", async () => {
    ...
    expect(statusHelp.stdout).toContain("run bb better-notes push separately");
  ...
  it("routes project-backed workspace files through the selected or primary source", async () => {
    ...
    await expect(
      host.harness.callRpc("openFile", {
        ...
        path: "better-notes/plan.md",
      }),
    ).resolves.toMatchObject({ previewPath: "better-notes/plan.md" });
    await expect(
      host.harness.callRpc("saveOpenedFile", {
        source: selectedSource,
        path: "better-notes/plan.md",
    ...
    await expect(
      ...
      previewPath: "better-notes/plan.md",
    });
    await expect(
      host.harness.callRpc("openFile", {
    ...
    expect(host.harness.sdk.callsTo("files.read")).toEqual([
      [
        {
          hostId: "host_remote",
          path: "/remote/project/better-notes/plan.md",
      ...
      [
        {
          hostId: "host_primary",
          path: "/primary/project/better-notes/plan.md",
