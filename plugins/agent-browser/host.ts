...
const preparing = "preparing the agent-browser runtime";
...
export function createHostEntry(
  ...
  async function attachRuntime(
    ...
    if (resolved && (resolved.chrome || !chrome)) {
    ...
    if (!job) {
      ...
      const state = { abort, waiters: 0, detail: preparing };
  ...
  /** Session work that ends the session when the runtime itself fails. */
  async function work<T>(
    sessionId: string,
    context: ExperimentalHostRpcContext,
    action: (runtime: RuntimeSession, signal: AbortSignal) => Promise<T>,
  ): Promise<T> {
  ...
  return experimental_defineHostEntry({
    ...
    handlers: {
      async prepare(input, context): Promise<RuntimeState> {
        context.signal.throwIfAborted();
        const { promise, detach } = await attachRuntime(context, input.chrome);
        try {
          ...
          // A job started for a desktop session skips Chrome; the next poll
          // starts one that prepares it.
          if (outcome && (outcome.chrome || !input.chrome))
            return { status: "ready", version: outcome.version };
          return { status: "installing", detail: job?.detail ?? preparing };
        } finally {
          detach();
        }
      },
      async open(input, context) {
        ...
        const local = input.connectionUrl === undefined;
        const attached = await attachRuntime(context, local);
        ...
        if (local && !runtime.chrome)
          throw new Error(
            "Chrome is not prepared on this host yet; open the session again",
          );
        const abort = new AbortController();
        ...
        try {
          const created = await session.runtime;
      ...
      run(input, context) {
        ...
        return work(input.sessionId, context, async (runtime, signal) => ({
          output: await runtime.run(input.args, input.timeoutMs, signal),
          cursor: runtime.cursor,
        }));
      },
      screenshot(input, context) {
        return work(input.sessionId, context, (runtime, signal) =>
          runtime.screenshot(
            { full: input.full, annotate: input.annotate },
            signal,
          ),
        );
      },
      recording(input, context) {
        return work(input.sessionId, context, (runtime, signal) =>
          runtime.recording(signal),
        );
      },
      async preview(input, context) {
        const session = owned(input.sessionId);
