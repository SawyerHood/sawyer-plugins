...
// Short enough that the agent's cursor reads as motion, not a slideshow.
const MIN_POLL_INTERVAL_MS = 120;
...
function BrowserPreviewDirective({
  ...
  if (!SESSION_ID_PATTERN.test(sessionId)) {
    return (
      <Notice>
        agent-browser-preview requires a valid session attribute, e.g.{" "}
        <code>::agent-browser-preview{'{session="…"}'}</code>
      </Notice>
    );
  }
  return (
...
export default definePluginApp((app) => {
  ...
  app.slots.experimental_appOverlay({
    id: "agent-browser-preview-lightbox",
