...
/**
 * Pages the agent never works in. agent-browser's Chrome keeps a hidden
 * `chrome://newtab/` page next to the session's own, and Chrome sends no
 * screencast frames for it.
 */
function internalPage(url: string): boolean {
  return (
    url === "" ||
    url === "about:blank" ||
    internalSchemes.some((scheme) => url.startsWith(scheme))
  );
}
...
export function createPreview(
  ...
  const targets = new Map<
    string,
    { url: string; title: string; activity: number; hidden: boolean }
  >();
  const calls = new Map<
    number,
  ...
  /** The visible tab first, then a real page over an internal one, then the newest. */
  function bestTarget(): string | null {
    ...
    for (const [targetId, target] of targets) {
      const rank =
        (target.hidden ? 0 : 2) + (internalPage(target.url) ? 0 : 1);
      if (
        !best ||
        rank > best.rank ||
  ...
  function receive(connection: PreviewSocket, raw: string) {
    ...
    if (
      ...
      cast = null;
      select(connection);
    } else if (message.method === "Page.screencastVisibilityChanged") {
      if (message.sessionId === undefined || message.sessionId !== cast?.sessionId)
        return;
      const target = targets.get(cast.targetId);
      if (!target) return;
      target.hidden = !visibilitySchema.parse(message.params).visible;
      if (!target.hidden) return;
      // The agent switched tabs, so another page may be the visible one now.
      // Rate-limited so a browser with no visible tab cannot flip forever.
      if (Date.now() - lastUnhideAt >= timing.unhideMs) {
        lastUnhideAt = Date.now();
        for (const other of targets.values())
          if (other !== target) other.hidden = false;
      }
      select(connection);
