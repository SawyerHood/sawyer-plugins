...
const release: RuntimeRelease = {
  ...
  repository: "example/agent-browser",
  commit,
  integrity: tarballIntegrity,
  artifacts: { [platform]: sha256(binaryContent) },
};
const attestationPath = `/-/npm/v1/attestations/agent-browser@${version}`;
...
function provenance(
  ...
  const statement = {
    subject: [
      { name: `pkg:npm/agent-browser@${version}`, digest: { sha512: digest } },
    ],
    predicate: {
      buildDefinition: {
        externalParameters: {
...
const validProvenance = provenance("example/agent-browser", commit);
...
if (process.argv[2] === "install") {
  ...
  // The real tarball ships every platform binary, unpacked without an execute bit.
  if (config.binary !== null) fs.writeFileSync(path.join(dir, "agent-browser-" + config.platform), config.binary, { mode: 0o644 });
  fs.writeFileSync(path.join(dir, "..", "package.json"), JSON.stringify({ name: "agent-browser", version: config.version }));
  fs.writeFileSync(path.join(process.cwd(), "node_modules", ".package-lock.json"), JSON.stringify({ packages: { "node_modules/agent-browser": { version: config.version, resolved: (config.resolvedBase ?? process.env.npm_config_registry) + "/agent-browser/-/agent-browser.tgz", integrity: config.integrity } } }));
  process.exit(0);
}
...
beforeAll(async () => {
  fixture = await mkdtemp(join(tmpdir(), "bb-agent-browser-installer-"));
...
beforeEach(async () => {
  requests = [];
  served = { [attestationPath]: JSON.stringify(validProvenance) };
  await configureNpm({});
});
...
describe("runtime installer", () => {
  it("installs the exact pinned package once and reuses it without npm or network", async () => {
    ...
    expect(installed.binary).toBe(
      join(
        ...
        `node_modules/agent-browser/bin/agent-browser-${platform}`,
      ),
    );
    ...
    expect(await entries(dir)).toEqual([`agent-browser@${version}`]);
    const warm = await install(dir, {
      env: env(false),
      fetchAttestations: async () => {
        throw new Error("warm starts stay offline");
      },
    });
  ...
  it("refuses a binary whose digest differs from the pin and leaves nothing behind", async () => {
    const dir = await dataDir();
    await configureNpm({
      binary: binaryContent.replace(version, "1.0.0-other"),
    });
    await expect(install(dir)).rejects.toThrow("not the pinned");
    expect(await entries(dir)).toEqual([]);
    await configureNpm({ binary: null });
    await expect(install(dir)).rejects.toThrow("digest absent");
    expect(await entries(dir)).toEqual([]);
  });
  it("refuses a tarball whose integrity differs from the pin before reading provenance", async () => {
    const dir = await dataDir();
    await configureNpm({
      integrity: `sha512-${createHash("sha512").update("other").digest("base64")}`,
    });
    await expect(install(dir)).rejects.toThrow("not the pinned sha512-");
  ...
  it("refuses a binary that reports another version", async () => {
    ...
    await configureNpm({ binary: other });
    await expect(
      install(dir, {
        release: {
          ...release,
          registry: base,
          artifacts: { [platform]: sha256(other) },
        },
      }),
    ).rejects.toThrow(`did not report version ${version}`);
    expect(await entries(dir)).toEqual([]);
  });
  it("refuses packages without provenance from the pinned repository commit", async () => {
    const dir = await dataDir();
    served[attestationPath] = JSON.stringify(
      provenance("someone-else/agent-browser", commit),
    );
    await expect(install(dir)).rejects.toThrow("provenance names");
    expect(requests).toEqual([attestationPath]);
    await configureNpm({
      audit: { invalid: [{ name: "agent-browser" }], missing: [] },
    });
    ...
    expect(() =>
      checkProvenance(
        provenance("example/agent-browser", "b".repeat(40)),
        release,
        tarballIntegrity,
      ),
    ).toThrow(`not example/agent-browser at ${commit}`);
    expect(() =>
    ...
    expect(() =>
      checkSignatures(
        { invalid: [], missing: [{ name: "agent-browser" }] },
        release,
      ),
  ...
  it("reports npm failures, a missing npm, and an unrecorded platform clearly", async () => {
    ...
    ).rejects.toThrow(`no verified binary recorded for ${platform}`);
    await expect(
      resolveRuntime({
        dataDir: dir,
        chrome: false,
  ...
  it("serves concurrent installs from one npm run and tolerates a stale lock", async () => {
    ...
    await writeFile(
      join(installRoot(dir), `agent-browser@${version}.lock`),
      "999999999",
    );
    const foreign = join(installRoot(dir), ".staging-agent-browser@9.9.9-other");
    const ours = join(installRoot(dir), `.staging-agent-browser@${version}-old`);
    await mkdir(foreign);
    await mkdir(ours);
    ...
    expect((await entries(dir)).sort()).toEqual([
      ".staging-agent-browser@9.9.9-other",
      `agent-browser@${version}`,
    ]);
  });
