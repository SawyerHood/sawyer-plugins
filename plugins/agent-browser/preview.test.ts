...
function createFakeBrowser(initial: FakePage[]) {
    ...
    send(raw: string) {
      ...
      queueMicrotask(() => {
        ...
        if (message.method === "Page.startScreencast") {
          const targetId = sessions.get(message.sessionId!)!;
          casts.push({
            targetId,
            session: message.sessionId!,
            maxWidth: message.params.maxWidth!,
          });
          // Chrome reports visibility on start and sends frames only when visible.
          this.event(
            "Page.screencastVisibilityChanged",
            { visible: !hidden.has(targetId) },
            message.sessionId,
          );
          if (!hidden.has(targetId)) browser.frame(targetId);
        }
      });
    }
    close() {
  ...
  const browser = {
    ...
    /** Makes `visible` the foreground tab, like the agent switching tabs. */
    show(visible: string) {
      for (const targetId of pages.keys())
        if (targetId === visible) hidden.delete(targetId);
...
describe("live preview target selection", () => {
  it("ignores agent-browser's hidden New Tab page, whichever order Chrome lists it in", async () => {
    for (const order of [
      [shop, newTab],
      [newTab, shop],
    ]) {
      const browser = createFakeBrowser(order);
      browser.hide("newtab");
      ...
      expect(browser.casts.map((cast) => cast.targetId)).toEqual(["shop"]);
      preview.close();
    }
  });
  it("prefers a real page over internal pages even when those navigated last", async () => {
    const browser = createFakeBrowser([shop]);
    const preview = createPreview("ws://127.0.0.1:1/x", timing, browser.open);
    await preview.next(0, 1_000, never, "thumbnail");
    browser.update(newTab, "Target.targetCreated");
    browser.update(
      { ...newTab, targetId: "devtools", url: "devtools://devtools/x" },
  ...
  it("moves to the tab the agent switched to when the cast page is hidden", async () => {
    const docs: FakePage = {
      targetId: "docs",
      type: "page",
      url: "https://docs.test/",
    ...
    const browser = createFakeBrowser([docs, shop]);
    browser.hide("docs");
    const preview = createPreview("ws://127.0.0.1:1/x", timing, browser.open);
    const first = await preview.next(0, 1_000, never, "thumbnail");
    expect(first).toMatchObject({ url: shop.url });
    browser.show("docs");
    const switched = await preview.next(
      first!.sequence,
      1_000,
      never,
      "thumbnail",
    );
    expect(switched).toMatchObject({ url: docs.url, title: "Docs" });
    expect(browser.castTargets()).toEqual(["docs"]);
    await new Promise((resolve) => setTimeout(resolve, 40));
    browser.show("shop");
    expect(
      await preview.next(switched!.sequence, 1_000, never, "thumbnail"),
    ).toMatchObject({ url: shop.url });
    preview.close();
  });
  it("settles instead of flipping when no tab is visible", async () => {
    const browser = createFakeBrowser([shop, { ...shop, targetId: "other" }]);
    browser.hide("shop");
    browser.hide("other");
    const preview = createPreview(
      "ws://127.0.0.1:1/x",
      { ...timing, unhideMs: 10_000 },
      browser.open,
    );
    expect(await preview.next(0, 60, never, "thumbnail")).toBeNull();
    await new Promise((resolve) => setTimeout(resolve, 40));
    expect(browser.casts.length).toBeLessThanOrEqual(3);
    const settled = browser.casts.length;
    await new Promise((resolve) => setTimeout(resolve, 40));
    expect(browser.casts.length).toBe(settled);
    preview.close();
  });
});
