...
Two folds at the bottom hold what is rarely touched. **Permission mode** is for threads started with `bb auto-dispatch spawn`, lowered where the machine or provider allows less; in the composer, the permission picker is yours. **Advanced** has the Jev model id for each gateway.
