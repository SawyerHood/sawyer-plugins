...
- **Detailed mode.** A setting that turns each row into a three-line card in the style of t3code: project with the thread's status glyph in the upper right, then the title, then the branch or pull request with the machine name and provider icon. Turn it on under **Settings → Plugins → Sawyer Sidebar**, or with `bb plugin config sawyer-sidebar set detailedRows true`.
